...
\begin{sol}
	...
	Moreover, $(a, n) \neq 1$ since $a \mid n$.
	By \exref{0.3.12}, there does not exist $c \in \z$ such that $ac \equiv 1 \bmod n$.
...
\begin{exercise}[1.4.7]
    ...
    \hint{Subtract the number of $2 \times 2$ matrices which are \emph{not} invertible from the total number of $2 \times 2$ matrices over $\f_p$.
	You may use the fact that a $2 \times 2$ matrix is not invertible if and only if one row is a multiple of the other.}
\end{exercise}

\begin{sol}
    Observe that there are $p^4$ total $2 \times 2$ matrices over $\f_p$.
...
\begin{sol}
	...
	Since $\gl_n(S) \subseteq \gl_n(F)$, then $\gl_n(F)$ is non-Abelian for all $n \geq 2$.
\end{sol}
...
\begin{solenum}
	...
	These are the matrices that have order 4:
        ...
        \]
    \end{itemize}
    \item Let $X \in H(\r)$ be non-identity.
	It is easy to see by induction that
    \[
    	X^n = (na, n(n - 1)ac/2 + nb, nc).
    \]
    Since $X \neq I_3$, then at least one of $a, b, c$ is nonzero.
	It is clear that $a \neq 0$ or $c \neq 0$ implies $X^n \neq I_3$.
	Now suppose $a = c = 0$ and $b \neq 0$.
	Then $X^n = (0, nb, 0)$.
	Since $b \neq 0$, then $nb \neq 0$ for all $n \in \zp$.
	Hence, $X^n \neq I_3$ for all $n \in \zp$.
	Therefore, every non-identity element of $H(\r)$ has infinite order.
\end{solenum}