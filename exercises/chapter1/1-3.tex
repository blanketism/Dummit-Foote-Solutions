...
\begin{solenum}
    ...
    \end{align*}
    Hence, the set of integers that produce a 12-cycle is the set $\set{i + 12k \mid i \in \{1, 5, 7, 11}, k \in \z\}$.
    \item $\set{i + 8k \mid i \in \{1, 3, 5, 7}, k \in \z\}$.
    \item $\set{i + 14k \mid i \in \{1, 3, 5, 9, 11, 13}, k \in \z\}$. \qh
\end{solenum}

\begin{exercise}[1.3.10]
    Prove that if $\sigma$ is the $m$-cycle $(a_1\ a_2\ \dots\ a_m)$, then for all $i \in \set{1,2,\dots,m}$, $\sigma^i(a_k) = a_{k+i}$, where $k+i$ is replaced by its least residue mod $m$ when $k+i > m$. Deduce that $|\sigma| = m$.
\end{exercise}
...
\begin{sol}
    ...
    To see that $|\sigma| = m$, note that $\sigma^m(a_k) = a_{k+m} = a_k$ for all $k \in \set{1,2,\dots,m}$, so that $\sigma^m = \id$. If $|\sigma| = i < m$, then $\sigma^i(a_1) = a_{1+i} = a_1$, which implies that $i \equiv 0 \bmod m$. Hence, $|\sigma| = m$.
\end{sol}
...
\begin{solenum}
    \item Put $\sigma = (1\ 3\ 5\ 7\ 9\ 2\ 4\ 6\ 8\ 10)$. Then $\sigma^2 = \tau$, so there exists a 10-cycle $\sigma$ such that $\tau = \sigma^k$ for $k = 2$.
    \item We first prove the following lemma:
    \begin{lemma}{$(ac, bc) = c(a, b)$ for $a, b, c \in \z$.}
...
\begin{sol}
    Recall that $S_3 = \set{1, (1\ 2), (1\ 3), (2\ 3), (1\ 2\ 3), (1\ 3\ 2)}$. Observe that $S_3$ contains no element of order 6, so it cannot be generated by a single element. We may deduce that $S_3$ must be generated by at least two elements. Put $\alpha = (1\ 2)$ and $\beta = (1\ 3)$. Then $\alpha^2 = \beta^2 = 1$ so that $|\alpha| = |\beta| = 2$. By direct calculation, we have $\alpha\beta = (1\ 3\ 2), \beta\alpha = (1\ 2\ 3)$, and $(\alpha\beta\alpha) = (2\ 3)$.

    Let $S = \gen{\alpha, \beta \mid \alpha^2 = \beta^2 = (\alpha\beta)^3 = 1}$. We claim that $S = S_3$. In the previous exercise, we saw that $|S| \geq 6$ since we produced 6 distinct elements. To see that there are no more, observe by the relations that elements of $S$ cannot contain consecutive powers of $\alpha$ or $\beta$ greater than 1. Then elements of $S$ are of the form $1, \alpha, \beta$, or a string of $\alpha\beta$'s. From $(\alpha\beta)^3 = 1$, we have
    \[
    	(\alpha\beta\alpha)(\beta\alpha\beta) = 1 \implies \alpha\beta\alpha = (\beta\alpha\beta)\inv = \beta\inv\alpha\inv\beta\inv = \beta\alpha\beta,
    \]
    where $\beta\inv = \beta$ and $\alpha\inv = \alpha$ since $|\alpha| = |\beta| = 2$. Since $\alpha\beta$ is already a distinct element, then we check
    \[
    	(\alpha\beta\alpha)\beta = (\beta\alpha\beta)\beta = \beta\alpha,
    \]
    which is an element already produced. The last case considers strings that begin with $\beta$, which is $\beta\alpha\beta = \alpha\beta\alpha$, which is also already produced. Hence, $|S| = 6$, so that $S = S_3$.
\end{sol}