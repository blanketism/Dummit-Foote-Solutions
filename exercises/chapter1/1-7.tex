...
\begin{solenum}
    ...
    \item Let $G_a$ denote the stabilizer of some fixed $a \in A$. If $g, h \in G_a$, we have
    \[
    	gh \cdot a = g \cdot (h \cdot a) = g \cdot a = a
    \]
...
\begin{solitem}
    \item [\rightimp] Suppose $G$ acts faithfully on $A$ with kernel $K$. Certainly, $1 \cdot a = a$ for all $a \in A$, hence $1 \in K$. If $g \in K$, then $g \cdot a = a$. Faithfulness of the action implies that $g = 1$, so that $K = \set{1}$.
...
\begin{sol}
    Let $V$ be a vector space over a field $F$. Let $F\unt$ act on $V$ by $r \cdot v = rv$ for all $r \in F\unt$ and $v \in V$. If $V = \set 0$ is trivial, then $r \cdot 0 = 0$ for all $r \in F\unt$, hence the action is not faithful. Otherwise, let $v \in V$ be nonzero. Then $r \cdot v = v$ implies $(r - 1)v = 0$. Since $v \neq 0$, then $r - 1 = 0$, or $r = 1$. Therefore, the action is faithful.
\end{sol}
...
\begin{solenum}
    \item Let $\sigma, \tau \in S_A$. Then for any cardinality $k$ subset $\set{x_1, \ldots, x_k}$ of $A$, we have
    \begin{align*}
    ...
    \end{align*}
    Moreover, $1 \cdot \set{x_1, \ldots, x_k} = \set{1(x_1), \ldots, 1(x_k)} = \set{x_1, \ldots, x_k}$. Then it is a group action.
    \item
    \[
        \begin{array}{c|c|c}
...
\begin{solenum}
    ...
    \item Let $\sigma \in S_n$ be a non-identity permutation. Then there exists some $a \in A$ such that $\sigma(a) \neq a$. Consider the ordered $k$-tuple $T = (a, x_2, x_3, \ldots, x_k)$. Then $\sigma(T) = (\sigma(a), \sigma(x_2), \ldots, \sigma(x_k)) \neq T$ because $\sigma a \neq a$. Therefore, no non-identity permutation fixes every ordered $k$-tuple of elements of $A$, and the action is faithful for all $1 \leq k \leq \abs A$.
\end{solenum}
...
\begin{sol}
    Let $\phi : D_8 \to S_4$ be the permutation representation of the action of $D_8$ on the vertices of a square $\set{1, 2, 3, 4}$, where the vertices are labeled in order around the square starting in the top left corner, then going clockwise. Then the cycle decompositions are as follows:
    \begin{align*}
...
\begin{sol}
    ...
    \]
    Let $P = \set{P_0, \ldots, P_{n/2 - 1}}$. Since $D_{2n}$ is the set of symmetries of an $n$-gon, then $g \cdot P_i = P_j$ for some $j$ for all $g \in D_{2n}$ and $P_i \in P$. Therefore, $D_{2n}$ acts on $P$.

    Observe that $r^k \cdot P_i = \set{i + k, i + k + n/2} = P_{i + k \bmod n/2}$. Then $r^k$ is in the kernel if $k \bmod n/2 \equiv 0$, or when $n/2 \mid k$. Since $k < n$, then $k$ is either $0$ or $n/2$. Then 1 and $r^{n/2}$ are in the kernel.

    On the other hand, since $n$ is even, we have two types of reflections. Let $s_v$ denote the reflection across the line through the vertices $i$ and $i + n/2$. Let $s_m$ denote the reflection across the line bisecting the edge formed by the vertices $i$ and $i + 1$. Then $s_v \cdot P_i = P_i$, while $s_v \cdot P_j = P_{(2i - j) \bmod n/2}$ for $j \neq i$. Then $j \equiv 2i - j \bmod n/2$, or $n/2 \mid 2(j - i)$. Since $j \neq i$ and $0 \leq i, j \leq n/2 - 1$, then $j - i \equiv n/4 \bmod n/2$. Then only $P_i$ is fixed by $s_v$ when $2 \mid n$, while both $P_i$ and $P_{i + n/4}$ are fixed when $4 \mid n$. Then $s_v$ is in the kernel when $n/2 \leq 2$, or $n = 4$.

    It is clear that $s_m$ does not have a free pair to fix. One may observe that $s_m \cdot P_j = P_{(2i + 1 - j) \bmod n/2}$, so $j \equiv 2i + 1 - j \bmod n/2$, or $n/2 \mid 2(j - i) + 1$. If $4 \mid n$, then $n/2$ is even while $2(j - 1) + 1$ is odd. If $4 \nmid n$, then we have at most one pair fixed, hence $s_m$ in the kernel when $n/2 \leq 1$ or $n = 2$.
...
\begin{exercise}[1.7.17]
    ...
    \[
    	 x \mapsto gxg^{-1}. 
    \]
    For fixed $g\in G$, prove that conjugation by $g$ is an isomorphism from $G$ onto itself. Deduce that $x$ and $gxg^{-1}$ have the same order for all $x\in G$ and that for any subset $A$ of $G$, $|A| = |gAg^{-1}|$ (here $gAg^{-1} = \set{gag^{-1} \mid a\in A}$).
\end{exercise}
...
\begin{sol}
    ...
    By \exref{1.6.2} and $\phi \in \aut(G)$, we have $|x| = |gxg\inv|$. Moreover, the bijection $\phi$ restricts to a bijection from $A$ to $gAg\inv$, so $|A| = |gAg\inv|$.
\end{sol}
...
\begin{solitem}
    ...
    \item Let $a, b \in A$ such that $a \sim b$. Then $a = hb$ for some $h \in H$. Then $b = h\inv a$, hence $b \sim a$.
...
\begin{sol}
    Let $G$ be the group of rigid motions of a tetrahedron with vertices labeled $1, 2, 3, 4$. Then each $\alpha \in G$ sends some vertex to another vertex so that $G$ acts on the set of vertices $A = \set{1, 2, 3, 4}$.
