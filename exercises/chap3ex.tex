...
\begin{specialexercise}\label{ex3.1.14}
...
\begin{exercise}\label{ex3.1.15}
    Prove that a quotient of a divisible abelian group by any proper subgroup is also divisible. Deduce that $\mathbb{Q}/\mathbb{Z}$ is divisible (cf. \hyperref[ex2.4.19]{Exercise 2.4.19}).
\end{exercise}
...
\begin{specialexercise}\label{ex3.1.16}
...
\begin{specialexercise}\label{ex3.1.20}
...
\begin{specialexercise}\label{ex3.1.40}
    Let $G$ be a group, let $N$ be a normal subgroup of $G$ and let $\bar{G} = G/N$. Prove that $\bar{x}$ and $\bar{y}$ commute in $\bar{G}$ if and only if $x^{-1}y^{-1}xy \in N$. (The element $x^{-1}y^{-1}xy$ is called the \emph{commutator} of $x$ and $y$ and is denoted by $[x,y]$.)
\end{specialexercise}
...
\begin{specialexercise}\label{ex3.1.41}
...
\begin{specialexercise}\label{ex3.1.43}
...
\begin{specialexercise}\label{ex3.2.5}
...
\begin{specialexercise}\label{ex3.2.7}
...
\begin{specialexercise}\label{ex3.2.9}
...
\begin{specialexercise}\label{ex3.2.10}
...
\begin{specialexercise}\label{ex3.2.11}
...
\begin{exercise}\label{ex3.2.16}
    Use Lagrange's Theorem in the multiplicative group $(\mathbb{Z}/p\mathbb{Z})^\times$ to prove \emph{Fermat's Little Theorem}: if $p$ is a prime then $a^p \equiv a \bmod p$ for all $a \in \z$.
\end{exercise}
...
\begin{specialexercise}\label{ex3.2.20}
...
\begin{exercise}\label{ex3.2.22}
    Use Lagrange's Theorem in the multiplicative group $(\mathbb{Z}/n\mathbb{Z})^\times$ to prove \emph{Euler's Theorem}: $a^{\varphi(n)} \equiv 1 \bmod n$ for every integer $a$ relatively prime to $n$, where $\varphi$ denotes Euler's $\varphi$-function.
\end{exercise}
...
\begin{specialexercise}\label{ex3.3.2}
...
\begin{exercise}\label{ex3.3.10}
    Generalize the preceding exercise as follows. A subgroup $H$ of a finite group $G$ is called a \emph{Hall subgroup} of $G$ if its index in $G$ is relatively prime to its order: $(|G : H|, |H|) = 1$. Prove that if $H$ is a Hall subgroup of $G$ and $N \nsub G$, then $H \cap N$ is a Hall subgroup of $N$ and $HN/N$ is a Hall subgroup of $G/N$.
\end{exercise}
...
\begin{specialexercise}\label{ex3.4.5}
