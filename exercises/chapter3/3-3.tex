...
\begin{exercise}[3.3.1]
	...
	Prove that $|\gl_N(F) : \speclin_n(F)| = q - 1$. [See \exref{3.1.35}.]
\end{exercise}

\begin{sol}
    In \exref{3.1.35}, we showed that $\speclin_n(F) \nsub \gl_n(F)$ and that $\gl_n(F)/\speclin_n(F) \cong F\unt$.
	Since $F$ is a finite field of order $q$, then $\abs{F\unt} = q - 1$.
	We then have that $|\gl_n(F) : \speclin_n(F)| = \abs{F\unt} = q - 1$.
\end{sol}
...
\begin{sol}
    Let $\ac = \set{A \leq G \mid N \subseteq A}$ be the set of subgroups of $G$ containing $N$.
	Let $\bar{\ac} = \set{\bar A \leq \bar G}$ be the set of subgroups of $\bar G = G/N$.
	Define the map
    \[
    	\phi : \ac \to \bar{\ac} \quad \text{by} \quad \phi(A) = \bar A = A/N
    \]
    We first show that $\phi(A) \leq \bar G$.
	Since $1 \in A$ for any $A \in \ac$, then $1N = N \in \bar A$ so that $\bar 1 \in \bar A$, hence $\bar A$ is nonempty.
	...
	Let $\bar A \in \bar{\ac}$, and let $\pi : G \to \bar G$ be the natural projection homomorphism.
	Consider $\pi\inv(\bar A)$.
	Since $\pi$ is a homomorphism, then $\pi\inv(\bar A) \leq G$.
	Moreover, since $\bar 1 \in \bar A$, then $N = \pi\inv(\bar 1) \subseteq \pi\inv(\bar A)$ so that $\pi\inv(\bar A) \in \ac$.
	Note that
    \[
    	\phi(\pi\inv(\bar A)) = \pi\inv(\bar A)/N = \bar A
    \]
    so that $\phi$ is surjective.
	To show that $\phi$ is injective, suppose $\phi(A) = \phi(B)$ for some $A, B \in \ac$.
...
\begin{exercise}[3.3.8]
    Let $p$ be a prime and let $G$ be the group of $p$-power roots  of $1$ in $\c$ (cf. \exref{2.4.18}).
