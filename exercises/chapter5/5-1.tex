...
\begin{sol}
    Note that $Q_8$ is non-Abelian, so we may use the idea of a diagonal subgroup from \exref{3.1.38}. In particular, let $H = \gen{(i, 1)} = \set{(1, 0), (i, 1), (-1, 2), (-i, 3)}$. Then $H$ is a subgroup of $Q_8 \times Z_4$ that is isomorphic to $\gen{i}$. However,
    \[
...
\begin{exercise}[5.1.17]
    ...
    \begin{subexercises}
        ...
        \item Let $I = \z^+$ and let $p_i$ be the $i$th integer prime. Show that if $G_i = \z/p_i\z$ for all $i \in \z^+$, then every element of the restricted direct product of the $G_i$'s has finite order but $\prod_{i \in \z^+} G_i$ has elements of infinite order. Show that in this example the restricted direct product is the torsion subgroup of the direct product (cf. \exref{2.1.16}).
    \end{subexercises}
\end{exercise}
...
\begin{solenum}
    ...
    \item Consider the group $\intmod[2] \times \z$. The element $(1, 0)$ has order 2, and the element $(0, 1)$ has infinite order.
