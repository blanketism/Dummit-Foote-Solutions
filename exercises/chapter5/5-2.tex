...
\begin{sol}
    It is easy to see that $kA = k\z \times \cdots \times k\z$.
	Since $\z$ is Abelian, then $k\z \nsub \z$.
...
\begin{spex}[5.2.14]
    For any group $G$ define the \emph{dual group} of $G$ (denoted $\wh{G}$) to be the set of all homomorphisms from $G$ into the multiplicative group of roots of unity in $\c$.
	Define a group operation in $\wh{G}$ by pointwise multiplication of functions: if $\chi, \psi$ are homomorphisms from $G$ into the group of roots of unity then $\chi\psi$ is the homomorphism given by $(\chi\psi)(g) = \chi(g)\psi(g)$ for all $g \in G$, where the latter multiplication takes place in $\c$.
    \begin{subexercises}
...
\begin{solenum}
    \item Multiplication in $\c$ is already associative and commutative, so pointwise multiplication of functions is also associative and commutative.
	The map $\ep : G \to \c$ given by $\ep(g) = 1$ for all $g \in G$ is a homomorphism, since $\ep(gh) = 1 = 1 \cdot 1 = \ep(g)\ep(h)$ for all $g, h \in G$, and 1 is a root of unity.
	...
	Finally, if $\chi \in \wh{G}$, then the map $\chi^{-1} : G \to \c$ given by $\chi^{-1}(g) = \chi(g)^{-1}$ is a homomorphism, since $\chi^{-1}(gh) = \chi(gh)^{-1} = (\chi(g)\chi(h))^{-1} = \chi(g)^{-1}\chi(h)^{-1} = \chi^{-1}(g)\chi^{-1}(h)$ for all $g, h \in G$, and $\chi(g)^{-1}$ is a root of unity since $\chi(g)$ is a root of unity.
...
\begin{sol}
	...
	For $r > 0$, observe that $\z$ is not divisible, since there is no $x \in \z$ such that $2x = 1$.
