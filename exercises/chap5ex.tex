...
\begin{specialexercise}\label{ex5.1.12}
    Let $A$ and $B$ be groups. Assume $Z(A)$ contains a subgroup $Z_1$ and $Z(B)$ contains a subgroup $Z_2$ with $Z_1 \cong Z_2$. Let this isomorphism be given by the map $x_i \mapsto y_i$ for all $x_i \in Z_1$. A \textbf{\emph{central product}} of $A$ and $B$ is a quotient
...
\begin{sol}
    The base case of $n = 2$ is clear, since $P \cong Z_p \times Z_p$ is a non-cyclic $p$-group of order $p^2$. Let $|P| = p^n$ for some $n > 2$, and assume the result is true for all non-cyclic $p$-groups of order less than $p^n$. Let $Z \leq Z(P)$ be a subgroup of order $p$, and let $\bbar P = P/Z$. If $\bbar P$ is cyclic, then by \hyperref[ex3.1.36]{Exercise 3.1.36}, we have that $P$ is abelian, hence $P$ has at least 2 invariant factors. Then the subgroup generated by the elements of order $p$ in $P$ is a normal subgroup of order $p^2$, which is isomorphic to $Z_p \times Z_p$. If $\bbar P$ is not cyclic, then by induction, there exists a normal subgroup $\bbar H \leq \bbar P$ with $\bbar H \cong Z_p \times Z_p$. Let $H$ be the complete preimage of $\bbar H$ in $P$, so that $|H| = p^3$. Let
    \[H_0 = \set{x \in H \mid x^p = 1}.\]
    Then $H_0$ is a characteristic subgroup of $H$ of order $p^2$ or $p^3$ by \hyperref[ex5.4.9]{Exercise 5.4.9}. If $|H_0| = p^2$, then $H_0$ is characteristic in $H$ where $H \nsub P$, so $H_0 \nsub P$, and $H_0 \cong Z_p \times Z_p$, and we can take $U = H_0$. If $|H_0| = p^3$, let $P$ act on $H$ by conjugation. Since $Z \leq Z(P)$, then we may have $P$ act on $\bbar H$ by conjugation. We use the class equation, but we instead consider the action of $P$ on $\bbar H$ by conjugation. Since $|\bbar H| = p^2$, then the class equation gives
    \[|\bbar H| = |C_{\bbar H}(P)| + \sum_{i = 1}^k |P : C_P(\dbar x[i])|,\]
    where $\dbar x[1], \ldots, \dbar x[k]$ are representatives of the non-singleton orbits of the $P$-conjugation on $\bbar H$. Note that $|P : C_P(\dbar x[i])|$ is a power of $p$, hence $|C_{\bbar H}(P)| \equiv |\bbar H| \equiv 0 \bmod p$. Since $\bbar 1 \in C_{\bbar H}(P)$, then $|C_{\bbar H}(P)| \geq p$. Then there exists some $\bbar h \in C_{\bbar H}(P)$ with $\bbar h \neq \bbar 1$. Let $h \in H$ be the preimage of $\bbar h$ in $H$. Then $h \notin Z$, and $h^p \in Z$. Let $U = \gen{h, Z}$. Then $|U| = p^2$. Since $Z \nsub P$ is clear, we observe that for any $g \in P$, then $ghg\inv \in \bbar h \subseteq U$, hence $U \nsub P$. Moreover, $h^p = 1$, since $H_0 = H$, and $|h| = p$ with $h \not\in Z$, hence $\gen h \cap Z = 1$. Lastly, $Z \leq Z(P)$ implies that $h$ commutes with all of $Z$. Then $U \cong Z_p \times Z_p$, and we are done.
\end{sol}
