...
\begin{sol}
    Since $k \mid a$ and $k \mid b$, there exist $x, y \in \z$ such that $a = kx$ and $b = ky$. Then for any $s, t \in \z$, we have
    \[
...
\begin{sol}
    Since $n$ is composite, put $n = ab$ with $1 < a, b < n$. Then $n \mid ab$, but $n \nmid a$ and $n \nmid b$.
\end{sol}
...
\begin{sol}
    Suppose there existed $a, b$ such that $a^2 = pb^2$. If $(a, b) \neq 1$, divide both $a$ and $b$ by $(a, b)$ to get relatively prime integers. Then $p \mid a^2$, which implies $p \mid a$. Put $a = pk$ for $k \in \z$. Then
    \[
...
\begin{sol}
    In the product $n!$, a factor of $p$ appears in each of the integers $p, 2p, \ldots, \floor{n/p}p$. Hence, there are $\floor{n/p}$ factors of $p$ in $n!$. Similarly, a factor of $p^2$ appears in each of the integers $p^2, 2p^2, \ldots, \floor{n/p^2}p^2$, so there are $\floor{n/p^2}$ factors of $p^2$ in $n!$. This contributes an additional factor of $p$ that was counted only once in the previous step. Continuing this process, we find that the largest power of $p$ which divides $n!$ is
    \[
...
\begin{sol}
    ...
    \]
    Since each factor $p_i^{\alpha_i - 1}(p_i - 1)$ divides $N$, we have $p_i - 1 \leq N$, so $p_i \leq N + 1$, bounding the number of possible primes. For the exponents, $p_i^{\alpha_i - 1} \leq N$ gives $\alpha_i \leq 1 + \log_2 N$ since $p_i \geq 2$. With finitely many choices for each $p_i$ and $\alpha_i$, there are only finitely many $n$ with $\phi(n) = N$.

    If $\phi$ did not tend to infinity, there would exist some $M \in \zp$ such that $\phi(n) \leq M$ for infinitely many $n$. But then some value $N \leq M$ would satisfy $\phi(n) = N$ for infinitely many $n$, contradicting the above. Hence $\phi(n) \to \infty$ as $n \to \infty$.
\end{sol}
...
\begin{sol}
    Let $n \in \zp$ with $d \mid n$. Put their prime factorizations as
    \[
    	n = p_1^{\alpha_1} p_2^{\alpha_2} \cdots p_k^{\alpha_k}, \quad d = p_1^{\beta_1} p_2^{\beta_2} \cdots p_j^{\beta_j},
    \]
    where $j \leq k$ and $1 \leq \beta_i \leq \alpha_i$ for all $1 \leq i \leq j$. Then $\phi(d) \mid \phi(n)$, since
    \[
    	\frac{\phi(n)}{\phi(d)} = \prod_{i = 1}^j \frac{p_i^{\alpha_i - 1}(p_i - 1)}{p_i^{\beta_i - 1}(p_i - 1)} \cdot \prod_{i = j+1}^k p_i^{\alpha_i - 1}(p_i - 1) = \prod_{i = 1}^j p_i^{\alpha_i - \beta_i} \cdot \prod_{i = j+1}^k p_i^{\alpha_i - 1}(p_i - 1) \in \z. \qh
    \]
\end{sol}