...
\begin{sol}
    Note that $\bar x = \set{x + 18k \mid k \in \z}$.
	Explicitly, we have:
    \begin{align*}
...
\begin{sol}
    Let $m \in \z$.
	By the Division Algorithm, there exists unique $q, r \in \z$ such that
    \[
...
\begin{sol}
    We consider $a^2 + b^2 = 3c^2$ in mod 4.
	By \exref{0.3.7}, $a^2 + b^2$ is either 0, 1, or 2 mod 4.
	By \exref{0.3.6}, $c^2$ is either 0 or 1 mod 4, so that $3c^2$ is either 0 or 3 mod 4.
	Thus, both sides must be 0 mod 4.

    It is clear that $4 \mid 3 c^2$ implies $4 \mid c^2$, and hence $2 \mid c$.
	Observe that if $a$ and $b$ were both odd, then $a^2 + b^2 \equiv 2 \mod 4$.
	If one of $a$ or $b$ were odd and the other even, then $a^2 + b^2 \equiv 1 \mod 4$.
	Thus, both $a$ and $b$ must be even.
	Put $a = 2\alpha$, $b = 2\beta$, and $c = 2\gamma$ for nonzero $\alpha, \beta, \gamma \in \z$.
	Substituting into the original equation gives
    \[
    	 (2\alpha)^2 + (2\beta)^2 = 3(2\gamma)^2.
    \]
    We may divide by 4 to obtain $\alpha^2 + \beta^2 = 3\gamma^2$, which is the same form as the original equation but with integers with smaller absolute values.
	Repeating this process leads to an infinite descent, which is impossible.
	Therefore, there are no solutions in nonzero integers $a, b,$ and $c$.
\end{sol}
...
\begin{sol}
	...
	Then $ab \equiv 1 \bmod n$ implies the existence of $k \in \z$ such that $ab - 1 = kn$, or $ab - kn = 1$.
	Thus, $(a, n) = 1$.
        \item[\leftimp] By $(a, n) = 1$, there exists $x, y \in \z$ such that $ax + ny = 1$, or $ax = 1 - ny$.
...
\begin{exercise}[0.3.12]
    Let $n \in \z, n > 1$ and let $a \in \z$ with $1 \leq a \leq n$.
...
\begin{sol}
    ...
    \]
    If there existed some $c \in \z$ such that $ac \equiv 1 \bmod n$, then $(ac)b \equiv b \bmod n$.
...
\begin{sol}
    Since $(a, n) = 1$, there exists $c, x \in \z$ such that $ac + nx = 1$.
...
\begin{sol}
    Use the Euclidean Algorithm to find $x, y \in \z$ such that $ax + ny = 1$ for each pair $(a, n)$.
