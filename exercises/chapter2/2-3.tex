...
\begin{sol}
    ...
    so that $x^{a (r - s)} = 1$. By $\abs x = 36, $ we know $36 \mid a(r - s)$. Since $48 \mid (r -s)$, then there exists $k$ such that $36 \mid 48ak$, or $3 \mid 4ak$. Since $k$ is arbitrary, this must hold for all integers $k$. Hence, $3 \mid a$.
...
\begin{sol}
    ...
    so that by Proposition 2.3, then $|xy|$ divides $\ell$. Moreover, this is not true if $x$ and $y$ do not commute. If we consider $r, s \in D_8$, then $\abs r = 4$ and $\abs s = 2$, but $\abs{rs} = 2$ which 4 does not divide. Moreover, consider $rs, s \in D_8$. Then $\abs{rs} = 2$ and $\abs s = 2$, but $\abs{rss} = \abs r = 4 \neq \lcm(2, 2)$.
\end{sol}
