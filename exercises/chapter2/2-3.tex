...
\begin{sol}
    Note that $202 = 2 \cdot 101$, which are both prime numbers. Then every number between 1 and 202 except 101 and even numbers is a generator of $\intmod[202]$.
\end{sol}
...
\begin{sol}
    ...
    \]
    so that $x^{a (r - s)} = 1$ whenever $48 \mid (r - s)$. This occurs if and only if $36 \mid a(r - s)$ whenever $48 \mid (r - s)$. Since $(36, 48) = 12$, then $36/12 = 3 \mid a$. Hence, $\psi_a$ is well-defined if and only if $3 \mid a$.

    Surjectivity requires that $x^a$ generates $Z_{36}$. By Proposition 2.5, this occurs if and only if $(a, 36) = 1$. But if $3 \mid a$, then $(a, 36) \geq 3$, so that $\psi_a$ is never surjective.
\end{sol}
...
\begin{sol}
    ...
    \]
    so that by Proposition 2.3, then $|xy|$ divides $\ell$. Moreover, this is not true if $x$ and $y$ do not commute. If we consider $x = rs$ and $y = s$ in $D_8$, then $|xy| = |rss| = 4$, but $\lcm(|x|, |y|) = \lcm(2, 2)$. Moreover, consider $x = y = r^2 \in D_8$. Then $\abs x = \abs y = 2$, but $\abs{xy} = \abs{r^4} = 1 \neq \lcm(2, 2) = 2$.
\end{sol}
