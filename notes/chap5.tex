...
\begin{enumerate}
    \item Suppose $A \nsub G$. If $A \cong Z_4$, then $\aut(V) \cong Z_2$, hence no nontrivial homomorphism from $B$ to $\aut(A)$ exists. Hence, $G \cong Z_{12}$.
    
    If $A \cong V_4$ instead, then $\aut(A) \cong S_3$, and there is a unique order 3 subgroup of $\aut(A)$; put it as $\gen\tau$. If $T = \gen t$, then we have three homomorphisms
    \[\phi_i : T \to \aut(A), \quad \phi_i(t) = \tau^i, \quad i = 0, 1, 2.\]
    Recall $\phi_0$ is trivial, while $\phi_1$ and $\phi_2$ give isomorphic semidirect products as they only differ by the choice of generator of $T$, either $t$ or $t^2 = t\inv$. Hence, the unique nonabelian group of order 12 with $A \nsub G$ is $V_4 \semidir T \cong A_4$.
